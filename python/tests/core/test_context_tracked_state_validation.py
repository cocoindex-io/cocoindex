...
def _create_env(db_name: str, embedder: StatefulEmbedder) -> coco.Environment:
...
def _run_app(db_name: str, embedder: StatefulEmbedder) -> None:
    env = _create_env(db_name, embedder)
    app = coco.App(
        coco.AppConfig(name=db_name, environment=env),
        _run_embed,
    )
    app.update_blocking()
...
def test_tracked_context_state_validation_function_level() -> None:
    ...
    Scenarios across runs (same env db):
    ...
    4. State goes back → memo_valid=False again → re-executes.
    """
    GlobalDictTarget.store.clear()
    _source_fn.clear()
    _metrics_fn.clear()

    db = "test_ctx_tracked_state_fn"
    _source_fn["a"] = "hello"
    _source_fn["b"] = "world"

    # Run 1: cache miss, both execute
    _run_app(db, StatefulEmbedder(name="e1", state_value=100))
    ...
    # Run 3: same embedder name (same fingerprint), state changes
    #        → __coco_memo_state__ returns memo_valid=False → re-execute.
    _run_app(db, StatefulEmbedder(name="e1", state_value=101))
    assert _metrics_fn.collect() == {"embed": 2}

    # Run 4: same embedder, same new state → memo valid again, 0 executions.
    _run_app(db, StatefulEmbedder(name="e1", state_value=101))
    assert _metrics_fn.collect() == {}
...
def _run_two_level_app(db: str, emb: TwoLevelStatefulEmbedder) -> None:
    ctx = coco.ContextProvider()
    ctx.provide(TWO_LEVEL_KEY, emb)
    settings = coco.Settings.from_env(db_path=get_env_db_path(db))
    env = coco.Environment(settings, context_provider=ctx)
    app = coco.App(coco.AppConfig(name=db, environment=env), _run_embed_two_level)
    app.update_blocking()
...
def test_tracked_context_state_valid_with_updated_state() -> None:
    ...
    db = "test_ctx_tracked_two_level"

    # Run 1: cache miss (no prev state).
    _run_two_level_app(db, TwoLevelStatefulEmbedder("x", generation=1, content_hash=10))
    ...
    _run_two_level_app(db, TwoLevelStatefulEmbedder("x", generation=2, content_hash=10))
    assert _metrics_two_level.collect() == {}

    # Run 4: another generation bump, hash still the same → still valid, 0 calls.
    _run_two_level_app(db, TwoLevelStatefulEmbedder("x", generation=3, content_hash=10))
    assert _metrics_two_level.collect() == {}

    # Run 5: content_hash changes → memo_valid=False → re-execute.
    _run_two_level_app(db, TwoLevelStatefulEmbedder("x", generation=4, content_hash=11))
    assert _metrics_two_level.collect() == {"embed2": 1}
...
def _embed_replace(key: str) -> str:
    ...
    emb = coco.use_context(EMBEDDER_KEY)
...
def _run_replace_app(db: str, emb: StatefulEmbedder) -> None:
    env = _create_env(db, emb)
    app = coco.App(coco.AppConfig(name=db, environment=env), _run_embed_replace)
    app.update_blocking()
...
def test_tracked_context_value_replaced() -> None:
    ...
    db = "test_ctx_tracked_replaced"

    _run_replace_app(db, StatefulEmbedder(name="e_old", state_value=100))
    ...
    _run_replace_app(db, StatefulEmbedder(name="e_new", state_value=100))
    assert _metrics_replace.collect() == {"embed3": 1}
...
def _run_composite_app(db: str, label: str, inner: _Inner) -> None:
    ctx = coco.ContextProvider()
    ctx.provide(COMPOSITE_KEY, (label, inner))
    settings = coco.Settings.from_env(db_path=get_env_db_path(db))
    env = coco.Environment(settings, context_provider=ctx)
    app = coco.App(coco.AppConfig(name=db, environment=env), _run_embed_composite)
    app.update_blocking()
...
def test_tracked_context_composite_state() -> None:
    ...
    db = "test_ctx_tracked_composite"

    _run_composite_app(db, "lbl", _Inner(name="i", state_value=1))
    ...
    _run_composite_app(db, "lbl", _Inner(name="i", state_value=2))
    assert _metrics_composite.collect() == {"embed4": 1}
...
async def _mount_embed() -> None:
    for item in _source_fn:
        await coco.mount(coco.component_subpath(item), _embed_component, item)


def _run_comp_app(db: str, emb: StatefulEmbedder) -> None:
    env = _create_env(db, emb)
    app = coco.App(coco.AppConfig(name=db, environment=env), _mount_embed)
    app.update_blocking()
...
def test_tracked_context_state_validation_component_level() -> None:
    ...
    _source_fn.clear()
    _metrics_comp.clear()
    db = "test_ctx_tracked_state_comp"
    _source_fn["a"] = ""
    _source_fn["b"] = ""

    _run_comp_app(db, StatefulEmbedder(name="ec", state_value=10))
    assert _metrics_comp.collect() == {"comp": 2}

    _run_comp_app(db, StatefulEmbedder(name="ec", state_value=10))
    assert _metrics_comp.collect() == {}

    _run_comp_app(db, StatefulEmbedder(name="ec", state_value=11))
    assert _metrics_comp.collect() == {"comp": 2}
...
def _run_branching_app(db: str, a: StatefulEmbedder, b: StatefulEmbedder) -> None:
    ctx = coco.ContextProvider()
    ctx.provide(KEY_A, a)
    ctx.provide(KEY_B, b)
    settings = coco.Settings.from_env(db_path=get_env_db_path(db))
    env = coco.Environment(settings, context_provider=ctx)
    app = coco.App(coco.AppConfig(name=db, environment=env), _run_branching)
    app.update_blocking()
...
def test_cache_hit_not_reusable_refreshes_context_fps() -> None:
    ...
    db = "test_ctx_tracked_branching"

    try:
        _use_both_keys = False
        _run_branching_app(
            db,
            StatefulEmbedder(name="a", state_value=1),
            StatefulEmbedder(name="b", state_value=10),
        )
        ...
        _run_branching_app(
            db,
            StatefulEmbedder(name="a", state_value=2),  # changed
            StatefulEmbedder(name="b", state_value=10),
        )
        assert _metrics_branch.collect() == {"branch": 1}
        ...
        _run_branching_app(
            db,
            StatefulEmbedder(name="a", state_value=2),
            StatefulEmbedder(name="b", state_value=11),  # changed
        )
        assert _metrics_branch.collect() == {"branch": 1}, (
