...
def test_context_provider_get_by_key_str() -> None:
    ...
    key = ContextKey[int]("test_get_by_key_str_unique_42", tracked=False)
...
def test_context_provider_get_by_key_str_with_type() -> None:
    ...
    key = ContextKey[int]("test_get_typed_unique_43", tracked=False)
    provider = ContextProvider()
    ...
    with pytest.raises(TypeError, match="expected int, got str"):
        str_key = ContextKey[str]("test_get_typed_wrong_type_44", tracked=False)
...
def test_context_key_coco_memo_key() -> None:
    ...
    key = ContextKey[int](some_unique_key, tracked=False)
