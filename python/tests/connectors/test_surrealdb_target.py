...
def _register_db(key: str, ns: str, db: str) -> Any:
    """Register a test SurrealDB database using stored config."""
    return surrealdb.register_db(
        key,
        url=_SURREALDB_URL,
        namespace=ns,
        database=db,
...
_surreal_db: Any = None  # surrealdb.SurrealDatabase
...
async def declare_table_and_rows() -> None:
    ...
    assert _surreal_db is not None
    ...
    table = await coco.use_mount(
        ...
        _surreal_db.mount_table_target,
        _table_name,
        schema,
...
async def test_create_table_schemafull_and_insert(
    ...
    global _source_rows, _row_type, _table_name, _table_schema, _surreal_db, _managed_by
    ...
    with _register_db("test_schemafull_db", ns, db) as db_handle:
        _surreal_db = db_handle
...
async def test_create_table_schemaless_and_insert(
    ...
    conn, ns, db = surreal_conn
    global _source_rows, _table_name, _surreal_db, _table_schema, _managed_by

    _table_name = "test_schemaless"
    _table_schema = None
    _managed_by = "system"

    with _register_db("test_schemaless_db", ns, db) as db_handle:
        _surreal_db = db_handle
...
async def test_update_record(
    ...
    with _register_db("test_update_db", ns, db) as db_handle:
        _surreal_db = db_handle
...
async def test_delete_record(
    ...
    with _register_db("test_delete_db", ns, db) as db_handle:
        _surreal_db = db_handle
...
async def test_no_op_when_unchanged(
    ...
    with _register_db("test_noop_db", ns, db) as db_handle:
        _surreal_db = db_handle
...
async def test_drop_table_on_removal(
    ...
    with _register_db("test_drop_db", ns, db) as db_handle:
        _surreal_db = db_handle
...
async def test_schema_evolution_add_field(
    ...
    with _register_db("test_evolve_add_db", ns, db) as db_handle:
        _surreal_db = db_handle
...
async def test_schema_evolution_remove_field(
    ...
    with _register_db("test_evolve_remove_db", ns, db) as db_handle:
        _surreal_db = db_handle
...
async def test_schema_evolution_change_id_type(
    ...
    with _register_db("test_evolve_id_db", ns, db) as db_handle:
        _surreal_db = db_handle
...
async def test_relation_basic(
    ...
    with _register_db("test_rel_basic_db", ns, db) as db_handle:
        ...
        async def declare_with_relations() -> None:
            ...
            likes_target = await coco.use_mount(  # type: ignore[call-overload]
                ...
                db_handle.mount_relation_target,
                "likes",
                person_target,
                post_target,
                likes_schema,
...
async def test_relation_delete(
    ...
    with _register_db("test_rel_del_db", ns, db) as db_handle:
        person_rows: list[Person] = []
        post_rows: list[Post] = []
        relation_rows: list[dict[str, Any]] = []
...
async def test_relation_without_schema(
    ...
    with _register_db("test_rel_schemaless_db", ns, db) as db_handle:
        person_rows: list[Person] = []
        post_rows: list[Post] = []
        relation_rows: list[dict[str, Any]] = []
...
async def test_relation_polymorphic(
    ...
    with _register_db("test_rel_poly_db", ns, db) as db_handle:
        person_rows: list[Person] = []
        org_rows: list[Org] = []
        post_rows: list[Post] = []
        relation_rows: list[dict[str, Any]] = []
...
async def test_relation_auto_id_with_update(
    ...
    with _register_db("test_rel_auto_id_db", ns, db) as db_handle:
        person_rows: list[Person] = []
        post_rows: list[Post] = []
        from_id_value = "alice"
...
async def test_relation_schema_without_id(
    ...
    with _register_db("test_rel_no_id_schema_db", ns, db) as db_handle:
        person_rows: list[Person] = []
        post_rows: list[Post] = []
        relation_rows: list[dict[str, Any]] = []
...
async def test_transaction_ordering(
    ...
    with _register_db("test_txn_order_db", ns, db) as db_handle:
        person_rows: list[Person] = []
        relation_rows: list[dict[str, Any]] = []
...
async def test_table_level_ordering(
    ...
    with _register_db("test_ddl_order_db", ns, db) as db_handle:
...
async def test_multiple_tables_shared_sink(
    ...
    with _register_db("test_shared_sink_db", ns, db) as db_handle:
        person_rows: list[Person] = []
        post_rows: list[Post] = []
...
async def test_vector_index_mtree(
    ...
    with _register_db("test_vec_mtree_db", ns, db) as db_handle:
        rows: list[VecRow] = []
...
async def test_vector_index_hnsw(
    ...
    with _register_db("test_vec_hnsw_db", ns, db) as db_handle:
        rows: list[VecRow] = []
...
async def test_vector_index_update(
    ...
    with _register_db("test_vec_update_db", ns, db) as db_handle:
        rows: list[VecRow] = []
...
async def test_user_managed_table(
    ...
    with _register_db("test_user_managed_db", ns, db) as db_handle:
        _surreal_db = db_handle
...
async def test_declare_row_alias(
    ...
    with _register_db("test_alias_db", ns, db) as db_handle:
...
async def test_schemaless_struct_input(
    ...
    with _register_db("test_struct_db", ns, db) as db_handle:
...
async def test_type_mapping(
    ...
    with _register_db("test_types_db", ns, db) as db_handle:
