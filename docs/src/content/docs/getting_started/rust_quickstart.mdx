...
For normal function-shaped components, prefer `use_mount!` and `mount_each!`.
Use `ctx.scope` or `ctx.mount_each` when you deliberately want an explicit-key
closure without the macros' function-and-argument fingerprint.
