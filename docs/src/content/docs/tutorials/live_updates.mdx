---
title: "*Live updates*"
description: Combine refresh intervals with per-source change capture, from S3 SQS notifications to Google Drive polling, and let FlowLiveUpdater keep the target in sync.
meta:
  time: ~20 minutes
...
For more detailed information on the `FlowLiveUpdater` and other live update options, please refer to the [Run a Flow documentation](https://cocoindex.io/docs-v0/core/flow_methods#live-update).
