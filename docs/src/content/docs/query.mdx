---
title: "*Query* support"
description: Share embedding logic between indexing and search via transform flows, then register query handlers so CocoInsight and your app can hit the same code path.
meta:
...
1.  Extract a sub-flow with the shared transformation logic into a standalone function.
    ...
    *   You need to annotate data types for both inputs and outputs as type parameter for `cocoindex.DataSlice[T]`. See [data types](./core/data_types) for more details about supported data types.
