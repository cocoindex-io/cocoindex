---
title: CocoIndex *data types*
description: The CocoIndex type system — basic types, structs, KTables, LTables, and vectors — and how each maps to Python type annotations.
...
When you define a [custom function](/docs-v0/custom_ops/custom_functions), you need to annotate the data types of arguments and return values.
...
For example, for [`SplitRecursively` function](/docs-v0/ops/functions#splitrecursively), the `text` and `chunk_size` arguments are not optional. If the input value of either of them is *None*, the function will return *None*.
