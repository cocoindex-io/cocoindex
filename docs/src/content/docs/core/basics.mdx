...
description: The core CocoIndex concepts behind every flow — indexing flows, data types, operations, data scopes, and incremental updates.
---
...
See [Settings](settings#databaseconnectionspec) for configuring its location. The internal storage is managed by CocoIndex, see [Setup / drop flow](/docs-v0/core/flow_methods#setupdrop-flow) for more details.
