...
description: Provide CocoIndex settings — database connection, app namespace, and other process-level options — via environment variables, a settings function, or an explicit init call.
meta:
...
See also [flow definition docs](/docs-v0/core/flow_def#control-processing-concurrency) about why it's necessary to control processing concurrency, and how to configure it on a per-source basis.
