---
title: Add a *new built-in target*
description: Implement the Rust TargetFactoryBase trait to add a new built-in target to CocoIndex, handling setup state, setup change application, and data mutations with access to the export schema.
...
CocoIndex allows you to create [custom targets](/docs-v0/custom_ops/custom_targets) by Python SDK.
...
You're recommended to read the [custom target documentation](/docs-v0/custom_ops/custom_targets) first as a starting point.
...
*Users* provide a *Target Spec* when they export specific data (in the form of [Data Collector](/docs-v0/core/flow_def#data-collector)) to the target.
...
Both *Setup Key* and *Setup State* are persisted in CocoIndex's [internal storage](/docs-v0/core/basics#internal-storage), to keep track of targets' states.
...
-   [Postgres](/docs-v0/targets/postgres), see [related code](https://github.com/search?q=repo%3Acocoindex-io%2Fcocoindex+path%3A%2Ftarget%2F+Postgres&type=code).
    It provides a good example for targets with specific column types in the schema.

-   [Qdrant](/docs-v0/targets/qdrant), see [related code](https://github.com/search?q=repo%3Acocoindex-io%2Fcocoindex+path%3A%2Ftarget%2F+Qdrant&type=code).
