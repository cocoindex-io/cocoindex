...
description: >
  Read Postgres tables via PgTableSource and write with declare_table_target() —
  automatic schema inference, pgvector index management (ivfflat / hnsw), custom
  type overrides, and SQL attachments for triggers and indexes.
---
...
- `row_type` — Optional record type (dataclass, NamedTuple, msgspec.Struct, or Pydantic model) for automatic row conversion. When provided, `columns` (if specified) must be a subset of the record's fields.
...
Pass a record type (dataclass, NamedTuple, msgspec.Struct, or Pydantic model) to automatically convert rows. When `columns` is omitted, the record's field names are used:
...
- `row` — A row object (dict, dataclass, NamedTuple, msgspec.Struct, or Pydantic model). Must include all primary key columns.
...
Define the table structure using a Python class (dataclass, NamedTuple, msgspec.Struct, or Pydantic model):
