...
- `write_batch_size` — Optional maximum number of row actions per LanceDB write call. Leave as `None` to apply each CocoIndex sink batch as one write.
...
`write_batch_size` is a connector-level chunking control. CocoIndex may coalesce row actions from sibling components before invoking the LanceDB sink; this option limits how many of those actions are sent to LanceDB in each write.
