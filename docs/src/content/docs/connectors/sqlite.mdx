...
description: >
  Write rows to SQLite databases with optional sqlite-vec vector support via
  float[N] columns — covers regular tables and vec0 virtual tables for
  optimized vector similarity search, with auto-loading of the vec extension.
---
...
- `row` — A row object (dict, dataclass, NamedTuple, msgspec.Struct, or Pydantic model). Must include all primary key columns with non-`None` values. Dict rows may omit nullable non-primary-key columns; omitted values are written as SQL `NULL`.
