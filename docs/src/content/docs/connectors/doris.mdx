...
description: >
  Write rows to Apache Doris tables via stream load with automatic upserts and
  deletions, HNSW/IVF vector indexes, and inverted indexes for full-text search —
  with optional schema enforcement.
---
