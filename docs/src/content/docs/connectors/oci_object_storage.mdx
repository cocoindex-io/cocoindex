...
---
The `oci_object_storage` connector provides utilities for reading objects from Oracle Cloud Infrastructure (OCI) Object Storage buckets, with optional live-mode support backed by OCI Object Storage events delivered through OCI Streaming (Kafka-protocol compatible).
...
A `True` verdict caches the metadata so subsequent `size()` / `read()` calls do not re-probe. A `False` verdict caches only the existence flag — a subsequent bare `size()` / `read()` without `exists()` will re-probe and re-raise the 404. The expected use is: call `exists()` first, branch on the result.
...
The typical setup uses **OCI Streaming** (Kafka-protocol compatible). Configure an [OCI Events Rule](https://docs.oracle.com/en-us/iaas/Content/Events/Concepts/eventsoverview.htm) that forwards Object Storage events (`createobject`, `updateobject`, `deleteobject`) to an OCI Streaming stream, then consume them via the [Kafka connector](./kafka) and feed the bytes payload view into `live_stream`:
