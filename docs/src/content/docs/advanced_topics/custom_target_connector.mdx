---
title: Building a *custom target connector*
description: >
  Connect CocoIndex's declarative target-state system to external systems.
  Covers TargetHandler, tracking records, actions, sinks, root vs container
  targets, attachment providers, and idempotency best practices.
...
- `OptChildHandlerT`: The child handler type, or `None` for leaf targets. A container handler declares it as the third argument of its `reconcile()` return type, `TargetReconcileOutput[Action, TrackingRecord, ChildHandler]`; it types the provider you get from `declare_target_state_with_child()`.
...
The sink function receives `context_provider` as its first argument (for looking up connections from the environment), followed by a sequence of actions, and returns nothing:
...
A **container** target's sink is built with `from_fn_with_children()` / `from_async_fn_with_children()` instead. Its callback takes a third argument, `child_slots`, holding a `coco.ChildSlot` for every action whose target state was declared with a child, and fulfills each slot with the child handler. See [Implementing container targets](#implementing-container-targets).
...
`OptChildHandlerT` is the handler type of the child target states the action's sink fulfills (`None` for leaf targets). Nothing in the tuple depends on it; it is how a container handler declares its child handler type.
...
2. **Resolution**: When the parent reconciles and its sink executes, CocoIndex hands the sink a `ChildSlot` for that action. The sink fulfills it with the child handler, and CocoIndex resolves the child provider with that handler.
...
The child handler often needs context from the parent's action execution. For example, a file handler needs to know the directory path that was created. By creating the handler inside the parent's sink, the handler has access to this runtime context.
...
When a container target state is no longer declared, CocoIndex runs the container handler's non-existence action and does **not** reconcile the children individually. The container action is therefore responsible for its children, in one of two shapes:

- **Destroy** — for a container CocoIndex owns, remove it together with its contents (`DROP TABLE`, `shutil.rmtree`, `drop_table`, …). The `_apply_dir_actions` sink in Step 2 below does this.
- **Abandon** — for a user-managed container (a message topic, a table the user created), leave the contents untouched. Once the container is no longer declared, CocoIndex has released its claim on it and must not write to it again: another writer may now own those keys, writing to a topic the user has since deleted would recreate it on brokers that auto-create topics, and the deletion policy captured from the old declaration (such as a `deletion_value_fn` closure) no longer exists.

The zvec connector (`python/cocoindex/connectors/zvec/_target.py`, `_apply_collection_actions`) implements both: it destroys system-managed collections and leaves user-managed ones alone. `App.drop()` applies the same shapes to every root container — it drops tables and abandons topics without visiting rows or messages — so document on your connector page which shape it uses. The user-facing contract is in [How CocoIndex syncs target states](../programming_guide/target_state#how-cocoindex-syncs-target-states).
...
### Step 2: Fulfill child slots from the sink

The parent's sink is built with `from_fn_with_children()` (or `from_async_fn_with_children()`). Besides `context_provider` and `actions`, its callback receives `child_slots`: a mapping from the index of each action whose target state was declared with a child to the `coco.ChildSlot` for that child provider. The sink creates the container and fulfills the slot with the child handler:
...
def _apply_dir_actions(
    ...
    for i, action in enumerate(actions):
        if action.should_delete:
            # An orphan delete has no child provider, hence no slot.
            shutil.rmtree(action.path, ignore_errors=True)
        else:
            action.path.mkdir(parents=True, exist_ok=True)
            # Hand the created path to the child handler.
            child_slots[i].fulfill(_EntryHandler(action.path))


_dir_sink = coco.TargetActionSink.from_fn_with_children(_apply_dir_actions)
```

Every slot must be fulfilled exactly once before the sink returns; an unfulfilled slot fails the commit. Because each slot is tied to its action by CocoIndex, a handler can never be attached to the wrong target state. Actions without a child provider — orphan deletes, or leaf actions when one sink is shared between a container and its children — have no entry in `child_slots`, so a batch of leaf actions costs nothing extra.
...
### Migrating from `ChildTargetDef`

Before child slots, a container target's sink was built with `from_fn()` / `from_async_fn()` and returned a list of `coco.ChildTargetDef(handler=...)` (or `None`) index-aligned with `actions`. That contract is deprecated and will be removed in a future release. Sinks written against it still run and emit a `DeprecationWarning`, and the pre-slot annotations still type-check:

- `coco.TargetActionSink[Action, ChildHandler]` is accepted; the second type argument is deprecated and ignored (evaluating it at runtime warns). Write `coco.TargetActionSink[Action]`.
- `coco.TargetReconcileOutput[Action, TrackingRecord, ChildHandler]` is unchanged: the third argument is how a container handler declares its child handler type.

To migrate a container sink:

1. Build it with `from_fn_with_children()` / `from_async_fn_with_children()` instead of `from_fn()` / `from_async_fn()`.
2. Add the third positional parameter, `child_slots: Mapping[int, coco.ChildSlot[ChildHandler]]`, and return `None`.
3. Replace each `outputs[i] = coco.ChildTargetDef(handler=h)` with `child_slots[i].fulfill(h)`; drop the `None` entries and the `return outputs`.
...
_dir_sink = coco.TargetActionSink[_DirAction, _EntryHandler].from_fn(_apply_dir_actions)
...
Leaf sinks need no change. If a leaf sink shares its callback with a container level (one sink per database serving table DDL and row writes), migrate it the same way: leaf actions simply have no entry in `child_slots`.
...
_file_sink = coco.TargetActionSink[_FileAction].from_fn(_apply_actions)
