...
- `OptChildHandlerT`: The child handler type, or `None` for leaf targets. Nothing in `reconcile()` mentions it — it types the provider you get from `declare_target_state_with_child()` — so a container handler subclasses `coco.TargetHandler[Spec, TrackingRecord, ChildHandler]` explicitly.
...
```python
class TargetReconcileOutput(Generic[ActionT, TrackingRecordT], NamedTuple):
