...
- **Policy limits owned by the call site** — attempt caps and retry budgets. Exhausting them re-raises the last transient error, so your normal error handling still applies.
- **The ambient `coco.timeout(...)` deadline** — raises `coco.DeadlineExceededError`, and can only stop retries sooner, never extend them. It fires only if you actually set a timeout.

Adding a timeout therefore never increases retry effort, and removing one never decreases it. No retry attempt starts past any limit, and backoff sleeps never exceed the tightest remaining budget. Built-in operations (`LlmPairResolver`, LiteLLM embedding retries) share one helper with these semantics.
