...
description: ParseJson, SplitRecursively, EmbedText, ExtractByLlm, DetectProgrammingLanguage, and the rest of the standard library of transforms you can drop into any flow.
meta:
...
Return: [*KTable*](/docs-v0/core/data_types#ktable), each row represents a chunk, with the following sub fields:
...
* `llm_spec` (`cocoindex.LlmSpec`): The specification of the LLM to use. See [LLM Spec](/docs-v0/ai/llm#llm-spec) for more details.
* `output_type` (`type`): The type of the output. e.g. a dataclass type name. See [Data Types](/docs-v0/core/data_types) for all supported data types. The LLM will output values that match the schema of the type.
...
* `api_type` ([`cocoindex.LlmApiType`](/docs-v0/ai/llm#llm-api-types)): The type of LLM API to use for embedding.
...
Not all LLM APIs support text embedding. See the [LLM API Types table](/docs-v0/ai/llm#llm-api-types) for which APIs support text embedding functionality.
