...
description: >
  The @coco.fn decorator makes Python functions participate in CocoIndex's change
...
The pool spawns a worker per concurrent call up to a cap, reuses idle ones, and keeps them resident for the life of the process. Each is a separate interpreter holding its own CUDA context, costing hundreds of MB of device memory before your model allocates anything. A fractional runner like `coco.GPU(0.25)`, which lets four calls share a card, can therefore hold four of them.

Workers are not pinned to a device: `CUDA_VISIBLE_DEVICES` is not set per worker, so a worker serves whichever GPU each call was given, and a function that ignores `coco.current_gpu()` can reach a GPU it was not assigned.

Cancelling a call does not release its GPU immediately. A worker that has already started cannot be interrupted, so the capacity is held until it finishes; otherwise the next call would be admitted onto a card that is still busy. Cancelling again does not shorten that wait, for the same reason.

A dying worker takes the whole pool with it: `ProcessPoolExecutor` terminates every worker when one fails, so **other calls running at the same time can fail too**, not just the one that crashed. Failures surface as `BrokenProcessPool` and are not retried: the component fails, nothing is written or memoized for it, and the next `app.update()` retries it cleanly.
:::
