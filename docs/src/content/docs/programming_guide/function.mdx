...
:::note[Subprocess mode and the GPU assignment]
`coco.current_gpu()` reports the same assignment inside the subprocess as it does in-process, so a function can select its device the same way in both modes.

Subprocess mode still runs one call at a time: every call shares a single worker, whatever the pool admits. Read the assignment rather than assuming a device, because the worker is not pinned to one: `CUDA_VISIBLE_DEVICES` is not set per worker, so it serves whichever GPU each call was given, and a function that ignores `coco.current_gpu()` can reach a GPU it was not assigned. One worker touching several cards also holds a CUDA context on each, costing hundreds of MB apiece, so prefer in-process mode when spreading work across GPUs.

A call that kills its worker surfaces as `BrokenProcessPool` and is not retried. The component fails, nothing is written or memoized for it, and the next `app.update()` retries it cleanly.
:::
