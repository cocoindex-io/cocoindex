---
title: Sharing resources via *context*
description: >
  Use ContextKey, builder.provide(), and use_context() to share connections,
  models, and config across processing components. Covers change detection on
  context values and accessing context outside mount boundaries.
---
...
db = env.get_context(PG_DB)
```

`default_env()` is async. From synchronous code, start the environment with `coco.start_blocking()` and then read the value with `asyncio.run(coco.default_env()).get_context(PG_DB)`, or keep the pool yourself outside the lifespan (see the query section of `examples/text_embedding/main.py`).
