---
title: "*Qdrant* target"
description: Each collected row becomes a Qdrant point, with fixed-dimension vectors mapped to named dense or multi-vectors and everything else folded into the payload.
...
*   `connection` ([auth reference](/docs-v0/core/flow_def#auth-registry) to `QdrantConnection`, optional): The connection to the Qdrant instance. `QdrantConnection` has the following fields:
