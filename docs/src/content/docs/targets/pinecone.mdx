---
title: "*Pinecone* target"
description: Stream collected rows as Pinecone vectors with a single primary key, pick a cloud and region for serverless indexes, and batch upserts with one tunable parameter.
