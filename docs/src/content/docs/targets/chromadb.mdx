---
title: "*ChromaDB* target"
description: Map each collected row to a ChromaDB document with one embedding vector, route a chosen field into the document body, and let the rest land as metadata.
