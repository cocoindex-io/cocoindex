---
title: "*Postgres* (pgvector) target"
description: Land rows in a Postgres table with pgvector columns for fixed-dimension embeddings, per-column options, and optional custom schemas and table names.
...
* `database` ([auth reference](/docs-v0/core/flow_def#auth-registry) to `DatabaseConnectionSpec`, optional): The connection to the Postgres database.
    See [DatabaseConnectionSpec](/docs-v0/core/settings#databaseconnectionspec) for its specific fields.
    If not provided, will use the same database as the [internal storage](/docs-v0/core/basics#internal-storage).
