...
description: Authenticate with a service account, share the target folders, and CocoIndex will import files and poll for recent changes without re-listing the whole tree.
...
*   `max_file_size` (`int`, optional): when set, any source file exceeding the limit (in bytes) will be ignored.
    ...
    it's typically cheaper than a full refresh by setting the [refresh interval](/docs-v0/core/flow_def#refresh-interval) especially when the folder contains a large number of files.
...
The output is a [*KTable*](/docs-v0/core/data_types#ktable) with the following sub fields:
