...
description: Connect with a SAS token, an access key, or default Azure credentials and stream blobs from a container into a flow, with include and exclude glob patterns.
