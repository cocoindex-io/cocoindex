...
description: Point CocoIndex at an S3 bucket with IAM credentials, filter by prefix and glob patterns, and wire up an SQS queue for near-instant change notifications.
