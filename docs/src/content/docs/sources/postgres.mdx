...
description: Pull rows from any primary-keyed table, filter with a SQL WHERE expression, and use LISTEN/NOTIFY or an ordinal column to catch every insert, update, and delete.
...
The output is a [*KTable*](/docs-v0/core/data_types#ktable) with straightforward 1 to 1 mapping from Postgres table columns to CocoIndex table fields:
