...
This state-driven programming paradigm is already widely used in other areas:

- **Spreadsheets**: Each cell derives its value from input cells via a formula. When any upstream cell changes, downstream cells automatically recompute to reflect the new state.

- **React**: Your UI is a function of state. When state changes, React automatically re-renders the UI to match.

- **Materialized Views**: A materialized view's data is a function of its input tables. When input data changes, the view automatically refreshes to match.

:::

For data transformation, you read the current state from your source and perform a series of transformations. For example:

- Converting PDFs to markdown files

- Extracting features or structures

- Mapping data to fit a particular schema
