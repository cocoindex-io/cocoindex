...
Both `update()` and `update_async()` accept the following options:
...
```python
demo_flow.update(reexport_targets=True, print_stats=True)
...
await demo_flow.update_async(reexport_targets=True, print_stats=True)
...
```
