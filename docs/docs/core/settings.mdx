...
* `schema` (type: `Optional[str]`, default: `None`): The schema for the Postgres database. It sets the search path in database url to `schema` and public.

    *Environment variable* for `Settings.database.db_schema_name` : `COCOINDEX_DATABASE_SCHEMA_NAME` (Works only as a environment variable)
