...
* `db_schema_name` (type: `str`, optional): The database schema name to be used for storing CocoIndex internal metadata and tracking tables.
...
The `db_schema_name` field specifies the Postgres schema where CocoIndex internal tables (like metadata and tracking tables) will be stored.

If not set, CocoIndex will use the default schema of the database connection (usually `public`).

Note that the schema will be automatically created if it doesn't already exist.

*Environment variable*: `COCOINDEX_DATABASE_SCHEMA_NAME`

### DatabaseConnectionSpec

`DatabaseConnectionSpec` configures the connection to a Postgres database. It has the following fields:
...
### SurrealDB internal persistence (alternative)

If you set `COCOINDEX_SURREALDB_URL`, CocoIndex will use SurrealDB **instead of Postgres** for internal persistence (setup metadata + tracking state).

- Endpoint uses WebSocket, e.g. `ws://localhost:8000` (or `wss://...`).
- CocoIndex will use the namespace and database specified by env vars below.
- If both Surreal and Postgres env vars are set, **Surreal takes precedence**.

*Environment variables*:

* `COCOINDEX_SURREALDB_URL` (required): SurrealDB websocket endpoint, e.g. `ws://localhost:8000`
* `COCOINDEX_SURREALDB_NS` (required): namespace
* `COCOINDEX_SURREALDB_DB` (required): database
* `COCOINDEX_SURREALDB_USER` (optional): username (if not provided, no signin is performed)
* `COCOINDEX_SURREALDB_PASSWORD` (optional): password
...
| `COCOINDEX_SURREALDB_URL` | `surreal.endpoint` | No |
| `COCOINDEX_SURREALDB_NS` | `surreal.namespace` | No |
| `COCOINDEX_SURREALDB_DB` | `surreal.database` | No |
| `COCOINDEX_SURREALDB_USER` | `surreal.user` | No |
| `COCOINDEX_SURREALDB_PASSWORD` | `surreal.password` | No |
