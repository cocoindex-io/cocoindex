...
If you set `COCOINDEX_SURREAL_ENDPOINT`, CocoIndex will use SurrealDB **instead of Postgres** for internal persistence (setup metadata + tracking state).
...
* `COCOINDEX_SURREAL_ENDPOINT` (required): SurrealDB websocket endpoint, e.g. `ws://localhost:8000`
* `COCOINDEX_SURREAL_NS` (required): namespace
* `COCOINDEX_SURREAL_DB` (required): database
* `COCOINDEX_SURREAL_USER` (optional): username (if not provided, no signin is performed)
* `COCOINDEX_SURREAL_PASSWORD` (optional): password
...
| `COCOINDEX_SURREAL_ENDPOINT` | `surreal.endpoint` | No |
| `COCOINDEX_SURREAL_NS` | `surreal.namespace` | No |
| `COCOINDEX_SURREAL_DB` | `surreal.database` | No |
| `COCOINDEX_SURREAL_USER` | `surreal.user` | No |
| `COCOINDEX_SURREAL_PASSWORD` | `surreal.password` | No |
