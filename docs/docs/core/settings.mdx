...
* `database` (type: `DatabaseConnectionSpec`, optional): The connection to the Postgres database.
* `surreal` (type: `SurrealDbConnectionSpec`, optional): The connection to the SurrealDB database (alternative internal persistence backend).
...
If you set `COCOINDEX_SURREALDB_ENDPOINT`, CocoIndex will use SurrealDB **instead of Postgres** for internal persistence (setup metadata + tracking state).
...
* `COCOINDEX_SURREALDB_ENDPOINT` (required): SurrealDB websocket endpoint, e.g. `ws://localhost:8000`
...
| `COCOINDEX_DATABASE_URL` | `database.url` | Yes (No if you use SurrealDB) |
...
| `COCOINDEX_SURREALDB_ENDPOINT` | `surreal.endpoint` | No |
