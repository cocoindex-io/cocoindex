---
title: HTTP Server
description: Run the built-in HTTP server to expose CocoIndex APIs
---
...
- **Live updates from the CLI**: When running with `-L`, the server performs live updates while it’s up (similar to `cocoindex update -L`).
