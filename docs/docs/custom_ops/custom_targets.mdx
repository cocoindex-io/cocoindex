...
-   Setup changes. When users add a new target, delete an existing target, or make changes to the target spec,
    the framework will trigger the connector to apply these setup changes by calling `apply_setup_change()`.
-   Data changes. During the flow is running, when new rows-to-export appear, or existing ones are updated or deleted in the CocoIndex flow, the framework will trigger the connector to apply these data changes by calling `mutate()`.

The target connector needs to react to these changes and reflect them to the external system.
